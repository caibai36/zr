function [] = run_dpgmm_modified(train_mfcc_path, test_mfcc_path)
...
disp(['Processing train file: ' train_mfcc_path]);
disp(['Processing test file: ' test_mfcc_path]);

% eg: M = dlmread('../dpgmm/si1392.mfcc');
M = dlmread(train_mfcc_path);
data = M';
M_test = dlmread(test_mfcc_path);
data_test = M_test';
...
[z,post,cluster_ids]=run_dpgmm_subclusters_modified(data, data_test, initialClusters, dispOn, numProcessors, ...
...
% the frame label of clusters
dlmwrite(strcat(test_mfcc_path, '.dpmm.flabel'), z);
% the posteriorgram of each frame
dlmwrite(strcat(test_mfcc_path, '.dpmm.post'), post);
% the cluster label of posteriorgram in order 
% dlmwrite(strcat(test_mfcc_path, '.dpmm.post.clabel'), cluster_ids);
